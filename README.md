...
**Can Foundation Models Replace Feature Engineering?** A comparative study benchmarking N-HiTS and PatchTST against the "Window & Flatten" Random Forest and LSTM baselines for Significant Wave Height ($H_s$), Peak Wave Period ($T_p$), and Wave Direction ($Dir$).
...
* **Physical Consistency Constraint (Provisional):**
    * We explore hard-coding a physical constraint into the loss function: $\mathcal{L}_{total} = \mathcal{L}_{task} + \lambda \cdot \text{ReLU}(H_s/L - 0.142)$. This penalizes predictions where wave steepness exceeds the physical breaking limit ($1/7$), ensuring the model respects fluid dynamics boundaries.
